export function getChartColors() {
  ...
  return {
    ...
    tooltipBg: isDark ? 'rgba(10,10,30,0.92)' : 'rgba(255,255,255,0.95)',
    tooltipBorder: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)',
  };
}

export function getBarOptions(): any {
...
export function getRadarOptions(): any {
  const c = getChartColors();
  return {
    responsive: true,
    maintainAspectRatio: false,
...
export function getChartOptions(type: string = 'bar'): any {
  if (type === 'radar') return getRadarOptions();
  if (type === 'doughnut') return getDoughnutOptions();
